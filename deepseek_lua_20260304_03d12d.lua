...
local MyLibrary = {
    Themes = {
        ...
        -- Novos temas podem ser adicionados aqui
    },
    Info = {
        Version = "2.0.0"
    },
    Save = {
        ...
        TabSize = 160,
        Theme = "Main",
        Transparency = 0.03 -- nova opção de transparência
    },
    Settings = {},
    Connection = {},
    Instances = {},
    ...
    Icons = (function()
        return {
            ...
            -- ... (mesma lista de ícones, omitida para brevidade)
        }
    end)()
}
    ...
    local function Save(file)
        if readfile and isfile and isfile(file) then
            ...
            if type(decode) == "table" then
                ...
                if rawget(decode, "Transparency") then MyLibrary.Save["Transparency"] = decode["Transparency"] end
            end
        end
    end
    pcall(Save, "redz_library_V5.json")
end
...
local Connections, Connection = {}, MyLibrary.Connection do
    ...
    NewConnectionList({"FlagsChanged", "ThemeChanged", "FileSaved", "ThemeChanging", "OptionAdded", "Notification"})
end
...
function MyLibrary:SetTheme(NewTheme)
    ...
    SaveJson("redz_library_V5.json", MyLibrary.Save)
...
function MyLibrary:SetTransparency(Value)
    MyLibrary.Save.Transparency = Value
    SaveJson("redz_library_V5.json", MyLibrary.Save)
    -- Aplica a transparência ao fundo principal (pode ajustar conforme necessário)
...
function MyLibrary:Notify(Configs)
    ...
    local Duration = Configs.Duration or 3
    ...
    Make("Corner", NotificationHolder, UDim.new(0, 6))
    Make("Stroke", NotificationHolder)
    Make("Gradient", NotificationHolder, {Rotation = 60})

    local TitleLabel = InsertTheme(Create("TextLabel", NotificationHolder, {
    ...
    local TextLabel = InsertTheme(Create("TextLabel", NotificationHolder, {
        Size = UDim2.new(1, -10, 0, 20),
    ...
    local CloseBtn = Create("TextButton", NotificationHolder, {
    ...
    table.insert(MyLibrary.Notifications, NotificationHolder)
    Connection:FireConnection("Notification", Title, Text)
...
function MyLibrary:MakeWindow(Configs)
    ...
    local MainFrame = InsertTheme(Create("ImageButton", ScreenGui, {
        ...
        BackgroundTransparency = MyLibrary.Save.Transparency,
        Name = "Hub",
        Image = "rbxassetid://107918243025204",
        ImageColor3 = Color3.fromRGB(255, 255, 255),
        ImageTransparency = 0.2,
        ScaleType = Enum.ScaleType.Crop
    }), "Main")
    MyLibrary.MainFrame = MainFrame
    ...
    function Window:MakeTab(paste, Configs)
        ...
        function Tab:AddDropdown(Configs)
            ...
            local Searchable = Configs.Searchable or false -- nova opção
            ...
            local SearchBox
            if Searchable then
                SearchBox = Create("TextBox", DropFrame, {
                    Size = UDim2.new(1, -10, 0, 20),
                    Position = UDim2.new(0, 5, 0, 5),
            ...
            local ScrollFrame = InsertTheme(Create("ScrollingFrame", DropFrame, {
                ScrollBarImageColor3 = Theme["Color Theme"],
                Size = Searchable and UDim2.new(1, 0, 1, -30) or UDim2.new(1, 0, 1, 0),
                Position = Searchable and UDim2.new(0, 0, 0, 30) or UDim2.new(0, 0, 0, 0),
                ScrollBarThickness = 1.5,
                BackgroundTransparency = 1,
                BorderSizePixel = 0,
                CanvasSize = UDim2.new(),
            ...
            local function GetFrameSize()
                return UDim2.fromOffset(152, ScrollSize + (Searchable and 30 or 0))
            end
            ...
            local AddNewOptions, GetOptions, AddOption, RemoveOption, Selected, FilterOptions do
                ...
                FilterOptions = function(query)
                    query = query:lower()
                    for _, opt in pairs(Options) do
                        local visible = opt.Name:lower():find(query, 1, true) ~= nil or query == ""
                        opt.nodes[1].Visible = visible
                    end
                end
                for _, val in pairs(DOptions) do
                    AddOption(val, val)
                end
                CallbackSelected()
                UpdateSelected()
        ...
        function Tab:AddLabel(Configs)
            local Text = Configs[1] or Configs.Text or "Label"
            local Color = Configs.Color or Theme["Color Text"]
            local Font = Configs.Font or Enum.Font.Gotham
            local TextSize = Configs.TextSize or 12
            ...
            local This = {}
            function This:Set(NewText)
                Label.Text = GetStr(NewText)
            end
            function This:Visible(...) Funcs:ToggleVisible(Label, ...) end
            function This:Destroy() Label:Destroy() end
            return This
        end

        function Tab:AddKeybind(Configs)
            local Name = Configs[1] or Configs.Name or "Keybind"
            local Desc = Configs.Desc or ""
            local Default = Configs[2] or Configs.Default or Enum.KeyCode.F
            local Flag = Configs[3] or Configs.Flag or false
            local Callback = Funcs:GetCallback(Configs, 4)
            local ChangedCallback = Funcs:GetCallback(Configs, 5) or Callback
            local HoldToActive = Configs.Hold or false

            if CheckFlag(Flag) then Default = GetFlag(Flag) end
            local Button, LabelFunc = ButtonFrame(Container, Name, Desc, UDim2.new(1, -80))
            ...
            Make("Corner", KeyFrame, UDim.new(0, 4))
            ...
            local function UpdateKey(newKey)
                ...
                SetFlag(Flag, newKey.Name)
                Funcs:FireCallback(Callback, newKey)
            end
            ...
            Button.Activated:Connect(StartListening)
            ...
            function Keybind:SetKey(newKey)
                ...
                elseif type(newKey) == "string" then
                    -- Tenta converter string para Enum.KeyCode
                    for _, enum in pairs(Enum.KeyCode:GetEnumItems()) do
                        if enum.Name:lower() == newKey:lower() then
                            UpdateKey(enum)
                            break
                        end
                    end
            ...
            function Keybind:Callback(...) Funcs:InsertCallback(Callback, ...) end
            return Keybind
        end

        function Tab:AddColorPicker(Configs)
            local Name = Configs[1] or Configs.Name or "Color Picker"
            local Desc = Configs.Desc or ""
            ...
            local Button, LabelFunc = ButtonFrame(Container, Name, Desc, UDim2.new(1, -80))
            local ColorFrame = Create("Frame", Button, {
                Size = UDim2.new(0, 30, 0, 20),
                Position = UDim2.new(1, -10, 0.5),
                AnchorPoint = Vector2.new(1, 0.5),
                BackgroundColor3 = Default
            })
            Make("Corner", ColorFrame, UDim.new(0, 4))
            Make("Stroke", ColorFrame)

            -- Simples seletor de cor (pode ser expandido)
            local function OpenPicker()
                local picker = Window:Dialog({
                    Title = "Select Color",
                    Text = "Enter RGB values (0-255) separated by commas",
                    Options = {
                        { "OK", function()
